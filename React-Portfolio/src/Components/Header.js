        ...
        <ul className="dark:text-light-content font-medium md:flex items-center md:space-x-5 md:mr-10">
          <ul className="flex flex-col md:flex-row gap-4">
            {[
              ...
              <li key={to} className="text-xl pb-1 md:pb-0 relative group">
                <NavLink
                  to={to}
                  onClick={toggleClass}
                  className={({ isActive }) =>
                    isActive
                      ? "bg-gradient-to-r from-purple-500 to-blue-500 bg-clip-text text-transparent transition duration-200"
                      : "text-black transition duration-200 hover:bg-gradient-to-r hover:from-purple-500 hover:to-blue-500 hover:bg-clip-text hover:text-transparent"
                  }
                >
                  {label}
                </NavLink>
                <span
                  className={`absolute left-0 bottom-0 w-0 h-0.5 bg-gradient-to-r from-purple-500 to-blue-500 transition-all duration-300 group-hover:w-full ${
                    label === "Home" && "w-full"
                  }`}
                ></span>
              </li>
        ...
        <ul className="flex justify-evenly items-center my-5 md:my-0 md:space-x-5 md:mr-5">
          <li className="relative group">
            <a href={linkdein} target="_blank" rel="noreferrer noopener">
              <svg
                className="dark:fill-light-heading fill-dark-heading transition duration-300 group-hover:shadow-lg group-hover:shadow-gray-700/70 hover:scale-110 rounded-full"
                width="30"
                height="30"
                viewBox="0 0 30 30"
                fill="none"
